...
#---------------------------------------------------------------------------
# Build / Install / Deploy (Bash 腳本專案)
#---------------------------------------------------------------------------
...
build:
	@echo "🔨 Validating scripts syntax..."
	@bash -n $(INSTALL_SCRIPT) 2>/dev/null && echo "  ✅ $(INSTALL_SCRIPT)" || echo "  ❌ $(INSTALL_SCRIPT)"
	@for f in $(SCRIPTS_DIR)/*.sh; do \
		[ -f "$$f" ] && (bash -n "$$f" 2>/dev/null && echo "  ✅ $$f" || echo "  ❌ $$f"); \
	done 2>/dev/null || true
	@for f in $(BIN_DIR)/*; do \
		[ -f "$$f" ] && (bash -n "$$f" 2>/dev/null && echo "  ✅ $$f" || echo "  ❌ $$f"); \
	done 2>/dev/null || true
	@echo "🔨 Build (syntax check) complete."

clean:
	@echo "🧹 Cleaning temporary files..."
	rm -rf ./tmp/* 2>/dev/null || true
	rm -f /tmp/ghidra_download/*.zip 2>/dev/null || true
	@echo "🧹 Clean complete."

deploy:
	@echo "🚀 Running install script (dry-run check)..."
	@if [ ! -f $(INSTALL_SCRIPT) ]; then echo "❌ $(INSTALL_SCRIPT) not found"; exit 1; fi
	@echo "ℹ️  To deploy, run: sudo ./$(INSTALL_SCRIPT)"
	@echo "ℹ️  To deploy with verbose: sudo ./$(INSTALL_SCRIPT) --verbose"

logs:
	@tail -100 /var/log/ghidra-mcp-install.log 2>/dev/null || echo "⚠️  No install log found"
...
test:
	...
	@if command -v bats >/dev/null 2>&1; then \
		...
		echo "⚠️  bats 未安裝，請執行: sudo apt install bats"; \
		echo "ℹ️  Fallback: 執行語法檢查"; \
		$(MAKE) build; \
	fi
...
test-filter:
	...
	@bats tests/ -r --filter "$(FILTER)" 2>/dev/null || echo "⚠️  需要 bats: sudo apt install bats"

coverage:
	@echo "ℹ️  Bash 專案不支援 coverage，請使用 make test 執行測試"

lint:
	@echo "🔍 Linting shell scripts..."
	@if command -v shellcheck >/dev/null 2>&1; then \
		shellcheck -x $(INSTALL_SCRIPT) $(SCRIPTS_DIR)/*.sh $(BIN_DIR)/* 2>/dev/null; \
		echo "✅ shellcheck complete"; \
	else \
		echo "⚠️  shellcheck 未安裝，請執行: sudo apt install shellcheck"; \
	fi
